# VPN Key Management API - Документация

## Версия API: 2.3.3

### Обзор
API для управления VPN ключами с поддержкой VLESS+Reality протокола, индивидуальных портов и мониторинга трафика.

### Базовый URL
- API доступен по адресу сервера: `https://<IP_ИЛИ_ДОМЕН>:8000`
- Домен необязателен. Можно обращаться напрямую к публичному IP (пример: `https://203.0.113.10:8000`). Замените на адрес своего сервера.
- Если используете self-signed сертификат, добавьте флаг `-k/--insecure` в `curl` или установите доверенный сертификат.

### Аутентификация
Все запросы требуют API ключ в заголовке `X-API-Key`.

### Информационные эндпоинты (без аутентификации)
- `GET /` - Информация о версии и статусе API
- `GET /api/` - Информация об API
- `GET /health` - Health check для мониторинга состояния системы

---

## 🔑 Управление ключами

### Создание ключа
**POST** `/api/keys`

Создает новый VPN ключ с индивидуальным портом.

**Лимит:** 5 запросов/минуту

**Тело запроса:**
```json
{
    "name": "user@example.com"
}
```

**Ответ:**
```json
{
    "id": "uuid-key-id",
    "name": "user@example.com",
    "uuid": "vless-uuid",
    "created_at": "2025-08-05T12:00:00.000000",
    "is_active": true,
    "port": 10001,
    "short_id": "5f9c2b7adfe7d8ab",
    "sni": "www.microsoft.com"
}
```

Поле `short_id` появляется для новых ключей и используется в параметре `sid` VLESS-ссылки. Поле `sni` содержит случайно выбранный SNI домен из доступных ServerNames.

**Пример:**
```bash
curl -X POST "https://SERVER_ADDRESS:8000/api/keys" \
     -H "X-API-Key: your-api-key" \
     -H "Content-Type: application/json" \
     -d '{"name": "user@example.com"}'
```

### Получение списка ключей
**GET** `/api/keys`

Возвращает список всех VPN ключей.

**Лимит:** 30 запросов/минуту

**Ответ:**
```json
[
    {
        "id": "uuid-key-id",
        "name": "user@example.com",
        "uuid": "vless-uuid",
        "created_at": "2025-08-05T12:00:00.000000",
        "is_active": true,
        "port": 10001,
        "short_id": "5f9c2b7adfe7d8ab",
        "sni": "www.microsoft.com"
    }
]
```

### Получение конкретного ключа
**GET** `/api/keys/{key_id}`

Возвращает информацию о конкретном ключе.

**Лимит:** 60 запросов/минуту

**Параметры:**
- `key_id` - ID или UUID ключа

**Ответ:**
```json
{
    "id": "uuid-key-id",
    "name": "user@example.com",
    "uuid": "vless-uuid",
    "created_at": "2025-08-05T12:00:00.000000",
    "is_active": true,
    "port": 10001,
    "short_id": "5f9c2b7adfe7d8ab",
    "sni": "www.microsoft.com"
}
```

### Получение конфигурации ключа
**GET** `/api/keys/{key_id}/config`

Возвращает VLESS URL (одновременно в полях `client_config` и `vless_url`) и метаданные ключа.

**Параметры:**
- `key_id` - ID или UUID ключа

**Ответ:**
```json
{
    "key": {
        "id": "uuid-key-id",
        "name": "user@example.com",
        "uuid": "vless-uuid",
        "created_at": "2025-08-05T12:00:00.000000",
        "is_active": true,
        "port": 10001,
        "short_id": "5f9c2b7adfe7d8ab",
        "sni": "www.microsoft.com"
    },
    "client_config": "vless://uuid@SERVER_ADDRESS:PORT?security=reality&sni=www.microsoft.com&fp=chrome&pbk=public-key&sid=5f9c2b7adfe7d8ab&type=tcp&flow=xtls-rprx-vision#user@example.com",
    "vless_url": "vless://uuid@SERVER_ADDRESS:PORT?security=reality&sni=www.microsoft.com&fp=chrome&pbk=public-key&sid=5f9c2b7adfe7d8ab&type=tcp&flow=xtls-rprx-vision#user@example.com",
    "short_id": "5f9c2b7adfe7d8ab"
}
```

### Удаление ключа
**DELETE** `/api/keys/{key_id}`

Удаляет VPN ключ и освобождает порт.

**Лимит:** 10 запросов/минуту

**Параметры:**
- `key_id` - ID или UUID ключа

**Ответ:**
```json
{
    "message": "Key deleted successfully"
}
```

---

## 📊 Мониторинг трафика

> **Важно:** Система использует накопительный подсчет трафика. Данные обновляются автоматически каждые 5 минут через systemd timer, а также при каждом запросе к эндпоинту получения трафика.

### Получение накопительного трафика ключа
**GET** `/api/keys/{key_id}/traffic`

Возвращает накопительный трафик для конкретного ключа с момента создания или последнего сброса.

**Параметры:**
- `key_id` - ID или UUID ключа

**Ответ:**
```json
{
    "status": "success",
    "key_id": "uuid-key-id",
    "key_uuid": "vless-uuid",
    "total_bytes": 1234567,
    "timestamp": "2025-11-21T16:00:00"
}
```

**Примечание:** Перед возвратом данных автоматически обновляет статистику из Xray Stats API.

**Пример:**
```bash
curl -H "X-API-Key: your-api-key" \
    https://SERVER_ADDRESS:8000/api/keys/{key_id}/traffic
```

### Обнуление накопительного трафика ключа
**POST** `/api/keys/{key_id}/traffic/reset`

Обнуляет накопительный счетчик трафика для конкретного ключа.

**Параметры:**
- `key_id` - ID или UUID ключа

**Ответ:**
```json
{
    "status": "success",
    "message": "Traffic reset successfully",
    "key_id": "uuid-key-id",
    "key_uuid": "vless-uuid",
    "timestamp": "2025-11-21T16:00:00"
}
```

**Пример:**
```bash
curl -X POST -H "X-API-Key: your-api-key" \
    https://SERVER_ADDRESS:8000/api/keys/{key_id}/traffic/reset
```

---

## 🔧 Системные эндпоинты

### Синхронизация конфигурации
**POST** `/api/system/sync-config`

Принудительно синхронизирует конфигурацию Xray с базой ключей.

**Лимит:** 3 запроса/минуту

**Ответ:**
```json
{
    "message": "Configuration synchronized successfully",
    "status": "synced",
    "validation": {...},
    "timestamp": 1734883200
}
```

### Статус конфигурации
**GET** `/api/system/config-status`

Возвращает статус синхронизации конфигурации между SQLite и config.json.

**Ответ:**
```json
{
    "synchronized": true,
    "keys_json_count": 25,
    "config_json_count": 25,
    "keys_json_uuids": [...],
    "config_json_uuids": [...],
    "timestamp": 1734883200
}
```

### Проверка Reality настроек
**POST** `/api/system/verify-reality`

Проверяет корректность Reality настроек.

**Ответ:**
```json
{
    "message": "Reality settings verified and updated successfully",
    "status": "verified",
    "timestamp": 1734883200
}
```

### Статус портов
**GET** `/api/system/ports`

Возвращает статус назначения портов.

**Ответ:**
```json
{
    "port_assignments": {...},
    "used_ports": 25,
    "available_ports": 75,
    "max_ports": 100,
    "port_range": "10001-10100",
    "timestamp": 1734883200
}
```

### Сброс портов
**POST** `/api/system/ports/reset`

Сбрасывает все назначения портов.

**Ответ:**
```json
{
    "message": "All ports reset successfully",
    "status": "reset",
    "timestamp": 1734883200
}
```

### Валидация портов
**GET** `/api/system/ports/status`

Возвращает статус валидации портов.

**Ответ:**
```json
{
    "validation": {...},
    "timestamp": 1734883200
}
```

### Статус конфигурации Xray
**GET** `/api/system/xray/config-status`

Возвращает статус конфигурации Xray.

**Ответ:**
```json
{
    "config_status": {...},
    "timestamp": 1734883200
}
```

### Список inbound'ов Xray
**GET** `/api/system/xray/inbounds`

Возвращает информацию о VLESS inbound'ах согласно `config.json` (порт, UUID, shortIds).

**Ответ:**
```json
{
    "inbounds": [
        {
            "tag": "vless-inbound",
            "port": 10001,
            "client_count": 1,
            "uuids": ["vless-uuid"],
            "short_ids": ["5f9c2b7adfe7d8ab"],
            "dest": "www.microsoft.com:443",
            "server_names": ["www.microsoft.com"]
        }
    ],
    "timestamp": 1734883200,
    "source": "config.json"
}
```

### Синхронизация конфигурации Xray
**POST** `/api/system/xray/sync-config`

Синхронизирует конфигурацию Xray с ключами и автоматически перезапускает Xray.

**Ответ:**
```json
{
    "message": "Xray configuration synchronized successfully",
    "status": "synced",
    "short_id_sync": {...},
    "timestamp": 1734883200
}
```

### Валидация синхронизации Xray
**GET** `/api/system/xray/validate-sync`

Валидирует синхронизацию конфигурации Xray.

**Ответ:**
```json
{
    "validation": {...},
    "timestamp": 1734883200
}
```

### Исправление Reality ключей
**POST** `/api/system/fix-reality-keys`

Исправляет Reality ключи в конфигурации Xray и автоматически перезапускает Xray.

**Ответ:**
```json
{
    "status": "fixed",
    "message": "Reality keys fixed successfully",
    "timestamp": 1734883200
}
```

---

## 📝 Коды ошибок

- **200** - Успешный запрос
- **400** - Неверный запрос
- **401** - Неверный API ключ
- **404** - Ресурс не найден
- **500** - Внутренняя ошибка сервера
- **503** - Сервис недоступен (например, Xray Stats API)

---

## 🔄 Примеры использования

### Создание ключа
```bash
curl -X POST "https://SERVER_ADDRESS:8000/api/keys" \
     -H "X-API-Key: your-api-key" \
     -H "Content-Type: application/json" \
     -d '{"name": "user@example.com"}'
```

### Получение списка ключей
```bash
curl -H "X-API-Key: your-api-key" \
    https://SERVER_ADDRESS:8000/api/keys
```

### Получение конфигурации ключа
```bash
curl -H "X-API-Key: your-api-key" \
    https://SERVER_ADDRESS:8000/api/keys/{key_id}/config
```

### Получение накопительного трафика ключа
```bash
curl -H "X-API-Key: your-api-key" \
    https://SERVER_ADDRESS:8000/api/keys/{key_id}/traffic
```

### Обнуление трафика ключа
```bash
curl -X POST -H "X-API-Key: your-api-key" \
    https://SERVER_ADDRESS:8000/api/keys/{key_id}/traffic/reset
```

### Удаление ключа
```bash
curl -X DELETE -H "X-API-Key: your-api-key" \
    https://SERVER_ADDRESS:8000/api/keys/{key_id}
```

### Синхронизация конфигурации
```bash
curl -X POST -H "X-API-Key: your-api-key" \
    https://SERVER_ADDRESS:8000/api/system/sync-config
```

### Статус портов
```bash
curl -H "X-API-Key: your-api-key" \
    https://SERVER_ADDRESS:8000/api/system/ports
```

---

## 📊 Структура данных

### Ключ VPN
```json
{
    "id": "string",           // Уникальный ID ключа
    "name": "string",         // Имя/email пользователя
    "uuid": "string",         // UUID для VLESS протокола
    "created_at": "string",   // Дата создания (ISO 8601)
    "is_active": "boolean",   // Активен ли ключ
    "port": "integer",        // Назначенный порт
    "short_id": "string",    // Индивидуальный short_id (16 hex символов)
    "sni": "string"          // SNI домен для Reality протокола
}
```

### Трафик
```json
{
    "status": "success",
    "key_id": "string",       // ID ключа
    "key_uuid": "string",     // UUID ключа
    "total_bytes": "integer", // Накопительный трафик в байтах
    "timestamp": "string"     // Временная метка (ISO 8601)
}
```

---

## 🔍 Технические детали

### Подсчет трафика
- Трафик считается через Xray Stats API по UUID ключа
- Используется дельта-подсчет между снимками (uplink + downlink)
- Данные хранятся накопительно в SQLite базе данных
- Автоматическое обновление каждые 5 минут через systemd timer
- Обновление также происходит при запросе к эндпоинту получения трафика

### Индивидуальные порты
- Каждый ключ получает уникальный порт из диапазона 10001-10100
- Максимум 100 активных ключей
- Порт автоматически освобождается при удалении ключа

### Reality протокол
- Каждый ключ получает индивидуальный `short_id` (16 hex символов)
- SNI домен выбирается случайно при создании ключа из доступных ServerNames
- Поддержка VLESS+Reality с XTLS-RPRX-Vision flow

---

## 📌 Примечания

- Все эндпоинты (кроме `/`, `/api/` и `/health`) требуют аутентификации через заголовок `X-API-Key`
- Rate limiting применяется к некоторым эндпоинтам (указано в описании)
- Все эндпоинты возвращают JSON
- Временные метки возвращаются в формате ISO 8601 или Unix timestamp
- При использовании self-signed сертификата добавляйте флаг `-k` в curl команды
