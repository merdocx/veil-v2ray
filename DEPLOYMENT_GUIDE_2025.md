...
- После запуска `xray.service` выполняет `sync_inbounds.py`, который синхронизирует все ключи из `config/keys.json` через HandlerService. Повторный запуск доступен вручную:
  ...
  ```
- Если адрес или порт API меняются, задайте переменную `XRAY_API_SERVER` (формат `host:port`) для `vpn-api` и скриптов перед запуском.
...
### **6.3 Рекомендуемые SNI (Server Name Indication)**
При создании inbound'ов HandlerService автоматически использует популярные домены, чтобы трафик выглядел как обычный HTTPS. Для сервера в Нидерландах и пользователей в России рекомендуем держать следующий пул (при необходимости проверяйте доступность через `curl -I`):
