# VPN Key Management Server

Сервер для управления VPN ключами с поддержкой VLESS+Reality протокола, индивидуальных портов и мониторинга трафика.

## 🚀 Возможности

- ✅ **Управление ключами** - создание, удаление, просмотр VPN ключей
- 🔐 **VLESS+Reality** - современный протокол для обхода блокировок
- 🔌 **Индивидуальные порты** - каждый ключ получает уникальный порт (10001-10100)
- 📊 **Мониторинг трафика** - отслеживание трафика в реальном времени
- 📈 **Исторические данные** - накопление и анализ трафика с момента создания ключей
- 🔧 **API управление** - RESTful API для всех операций
- 🛡️ **Безопасность** - API ключ аутентификация, HTTPS

## 📦 Установка

### Требования
- Python 3.8+
- Xray-core v25.10.15+
- Nginx (для HTTPS)
- systemd

### ⚡ Быстрая установка (рекомендуется)
```bash
# Автоматическое развертывание
curl -O https://raw.githubusercontent.com/your-repo/vpn-server/main/deploy_auto.sh
chmod +x deploy_auto.sh
sudo ./deploy_auto.sh
```

### 📖 Ручная установка
```bash
# Клонирование репозитория
git clone <repository-url>
cd vpn-server

# Установка зависимостей
pip install -r requirements.txt

# Настройка переменных окружения
cp .env.example .env
# Отредактируйте .env файл

# Запуск сервисов
sudo systemctl enable vpn-api
sudo systemctl start vpn-api
sudo systemctl enable xray
sudo systemctl start xray
```

**📚 Подробные инструкции:** [DEPLOYMENT_GUIDE_2025.md](DEPLOYMENT_GUIDE_2025.md)

## 🔧 Конфигурация

### Переменные окружения (.env)
```bash
VPN_API_KEY=your-secret-api-key
VPN_HOST=0.0.0.0
VPN_PORT=8000
VPN_WORKERS=4
VPN_WORKER_MAX_REQUESTS=500
VPN_ENABLE_HTTPS=true
VPN_SSL_CERT_PATH=/etc/ssl/certs/vpn-api.crt
VPN_SSL_KEY_PATH=/etc/ssl/private/vpn-api.key
```

### Xray конфигурация
Основная конфигурация находится в `config/config.json` и автоматически обновляется при создании/удалении ключей.

## 📊 API Endpoints

### Управление ключами
- `POST /api/keys` - Создание ключа
- `GET /api/keys` - Список ключей
- `GET /api/keys/{id}` - Информация о ключе
- `GET /api/keys/{id}/config` - Конфигурация клиента
- `DELETE /api/keys/{id}` - Удаление ключа

### Мониторинг трафика
- `GET /api/traffic/simple` - Текущий трафик всех портов
- `GET /api/keys/{id}/traffic/simple` - Трафик конкретного ключа
- `POST /api/keys/{id}/traffic/simple/reset` - Сброс статистики

### Исторические данные
- `GET /api/traffic/history` - Общий объем трафика с момента создания
- `GET /api/keys/{id}/traffic/history` - История трафика ключа
- `GET /api/traffic/daily/{date}` - Ежедневная статистика
- `POST /api/keys/{id}/traffic/history/reset` - Сброс истории
- `POST /api/traffic/history/cleanup` - Очистка старых данных

### Месячная статистика
- `GET /api/traffic/monthly` - Месячная статистика всех ключей
- `GET /api/keys/{id}/traffic/monthly` - Месячная статистика ключа

### Системные
- `GET /health` - Health check (мониторинг состояния)
- `GET /api/system/ports` - Статус портов
- `POST /api/system/sync-config` - Синхронизация конфигурации
- `GET /api/system/config-status` - Статус конфигурации

## 📈 Мониторинг трафика

### Текущий трафик
Система отслеживает трафик в реальном времени на основе активных соединений:
- Количество соединений
- Общий объем трафика
- Входящий/исходящий трафик
- Скорость передачи данных

### Исторические данные
Новая система накопления данных позволяет:
- Отслеживать общий объем трафика с момента создания ключа
- Анализировать ежедневную статистику
- Просматривать активность по дням
- Получать детальную информацию о сессиях

**Пример получения исторических данных:**
```bash
# Общий объем трафика всех ключей
curl -H "X-API-Key: your-key" https://server:8000/api/traffic/history

# История конкретного ключа
curl -H "X-API-Key: your-key" https://server:8000/api/keys/key-id/traffic/history

# Ежедневная статистика
curl -H "X-API-Key: your-key" https://server:8000/api/traffic/daily/2025-08-05
```

## 🔌 Управление портами

Система автоматически назначает порты из диапазона 10001-10100:
- Каждый ключ получает уникальный порт
- Максимум 100 активных ключей
- Автоматическое освобождение при удалении ключа

## 🛠️ Управление сервисами

```bash
# VPN API сервис
sudo systemctl status vpn-api
sudo systemctl restart vpn-api
sudo systemctl stop vpn-api

# Xray сервис
sudo systemctl status xray
sudo systemctl restart xray
sudo systemctl stop xray

# Просмотр логов
sudo journalctl -u vpn-api -f
sudo journalctl -u xray -f
```

## 📁 Структура проекта

```
vpn-server/
├── api.py                      # Основной API сервер
├── port_manager.py             # Управление портами
├── xray_config_manager.py      # Управление конфигурацией Xray
├── simple_traffic_monitor.py   # Мониторинг трафика
├── traffic_history_manager.py  # Исторические данные трафика
├── generate_client_config.py   # Генерация клиентских конфигураций
├── data/
│   └── vpn.db                  # SQLite база (ключи, порты)
├── config/
│   └── config.json            # Конфигурация Xray
├── data/
│   └── vpn.db                 # SQLite база (ключи, порты, история)
├── requirements.txt           # Python зависимости
├── .env                      # Переменные окружения
└── README.md                 # Документация
```

## 🔒 Безопасность

- **API ключ аутентификация** - все запросы требуют валидный API ключ
- **HTTPS поддержка** - шифрование всех соединений
- **Изоляция портов** - каждый ключ на отдельном порту
- **Валидация данных** - проверка всех входных данных

## 📊 Мониторинг и логирование

### Логи сервисов
```bash
# VPN API логи
sudo journalctl -u vpn-api -f

# Xray логи
sudo journalctl -u xray -f

# Системные логи
sudo journalctl -f
```

### Файлы данных
- `data/vpn.db` - SQLite база данных (ключи, порты, история трафика)
- `config/config.json` - конфигурация Xray (автоматически обновляется при создании/удалении ключей)

### Поддержка и обслуживание
- Все данные хранятся в SQLite (`data/vpn.db`) - единственный источник истины
- Для быстрой проверки базы выполните `scripts/check_db_integrity.sh` (требуется `sqlite3`)
- Перед резервным копированием `scripts/backup.sh` автоматически запускает проверку `PRAGMA integrity_check`
- Перед включением systemd-сервиса выполните `scripts/setup_vpn_api_user.sh` (создаёт пользователя `vpnapi` и настраивает права)
- Для просмотра данных используйте `sqlite3 data/vpn.db` или API

## 🚨 Устранение неполадок

### Проблемы с API
1. Проверьте статус сервиса: `systemctl status vpn-api`
2. Проверьте логи: `journalctl -u vpn-api -f`
3. Убедитесь в правильности API ключа
4. Проверьте доступность порта 8000

### Проблемы с Xray
1. Проверьте статус: `systemctl status xray`
2. Проверьте конфигурацию: `xray test -c config/config.json`
3. Проверьте логи: `journalctl -u xray -f`

### Проблемы с портами
1. Проверьте назначения: `curl -H "X-API-Key: key" https://server:8000/api/system/ports`
2. Синхронизируйте конфигурацию: `curl -X POST -H "X-API-Key: key" https://server:8000/api/system/sync-config`

## 📞 Поддержка

При возникновении проблем:
1. Проверьте логи сервисов
2. Убедитесь в корректности конфигурации
3. Проверьте доступность всех зависимостей
4. Обратитесь к документации API

## 📝 Лицензия

Проект распространяется под лицензией MIT.

---

**Версия:** 2.3.6  
**Дата обновления:** 23 ноября 2025  
**Статус:** ✅ Актуально

### Последние изменения (v2.3.6)
- ✅ **Очистка проекта** - удалено 20 неактуальных файлов (устаревшие анализы, тестовые скрипты)
- ✅ **Обновлена документация** - проверена актуальность всех MD файлов
- ✅ **Оптимизирована структура проекта** - оставлены только актуальные файлы

### Предыдущие изменения (v2.3.5)
- ✅ **Очистка проекта** - удалено 28 устаревших файлов (анализы, тестовые скрипты)
- ✅ **Улучшена структура проекта** - оставлены только актуальные файлы

### Предыдущие изменения (v2.3.4)
- ✅ **Критические оптимизации для работы с до 100 пользователями**
- ✅ **Оптимизирована SQLite база данных** - busy_timeout и cache_size увеличены для стабильности
- ✅ **Увеличены bufferSize в Xray** - улучшение стабильности соединений при пиковых нагрузках на 10-15%
- ✅ **Увеличен timeout для Xray API** - устранение таймаутов при высокой нагрузке

### Предыдущие изменения (v2.2.8)
- ✅ **Исправлен systemd unit для Xray** - убраны проблемы с автозапуском, исправлены права доступа к логам
- ✅ **Упрощена конфигурация systemd** - удален ExecStartPost, синхронизация выполняется отдельно
- ✅ **Исправлены права доступа** - logs/ директория доступна для записи Xray

### Предыдущие изменения (v2.2.7)
- ✅ **Полный переход на SQLite** - удалены JSON-файлы, SQLite является единственным источником данных
- ✅ **Оптимизация производительности** - убраны избыточные операции синхронизации JSON
- ✅ **Упрощение архитектуры** - все модули используют только SQLite
- ✅ Обновлены все скрипты для работы с SQLite (sync_inbounds, update_traffic_stats, simple_traffic_monitor)
- ✅ Убран fallback на JSON в мониторинге
- ✅ Обновлен backup.sh - бэкапится только SQLite база

### Предыдущие изменения (v2.2.6)
- ✅ Миграция на SQLite для надежного хранения данных
- ✅ Мульти-воркер API (2 воркера) для улучшенной производительности
- ✅ Policy level 1 для новых ключей (оптимизация для мобильных устройств)
- ✅ Graceful shutdown для Xray
- ✅ Увеличен MemoryMax для Xray до 1GB
- ✅ Увеличен лимит соединений в firewall до 100
- ✅ Автозапуск Xray через systemd timer
- ✅ SSL termination на nginx для безопасности
- ✅ Rate limit отключен для бота (IP: 77.246.105.29)
- ✅ Исправлены права доступа для операций с ключами 
## ⚡ Оптимизации v2.3.1

Проект оптимизирован для работы с до 100 пользователей с максимальной стабильностью, скоростью и надежностью:

- ✅ Оптимизация TCP keepalive для стабильных VPN соединений
- ✅ Оптимизация TCP retries для быстрого восстановления
- ✅ Улучшена Restart политика для автоматического восстановления
- ✅ Увеличено время неактивных соединений для мобильных устройств
- ✅ Оптимизированы буферы для высокой нагрузки
- ✅ Увеличено количество API workers для лучшей производительности
- ✅ Оптимизированы DNS серверы для снижения задержек

**Результат:** +40-50% стабильность, производительность и надежность
