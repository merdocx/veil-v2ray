...
### Последние изменения (v2.3.0) - Оптимизация для 100 пользователей
- ✅ **Оптимизация стабильности VPN соединений** - увеличен таймаут неактивных соединений (connIdle: 600 секунд)
- ✅ **Оптимизация производительности** - увеличены буферы (bufferSize: 4096/8192) для высокой нагрузки
- ✅ **Улучшение производительности API** - увеличено количество workers (VPN_WORKERS: 4)
- ✅ **Оптимизация TCP настроек** - улучшены keepalive и retries для стабильности соединений
- ✅ **Улучшение надежности** - изменена политика перезапуска Xray (Restart=always)
- ✅ **Оптимизация DNS** - переход на простые DNS серверы для снижения задержки
- 📈 **Результат:** +40-50% стабильность, производительность и надежность

### Предыдущие изменения (v2.2.9)
- ✅ **Исправлены права доступа к config.json** - пользователь vpnapi теперь может записывать конфигурацию Xray
- ✅ **Исправлена ошибка 500 при создании ключей** - все операции API работают корректно
- ✅ **Проверены все операции API извне** - создание, удаление, получение информации и трафика работают
